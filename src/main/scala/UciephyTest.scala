...
class UciephyTestTL(params: UciephyTestParams, beatBytes: Int)(implicit p: Parameters) extends ClockSinkDomain(ClockSinkParameters())(p) {
  ...
  class UciephyTestImpl extends Impl {
    ...
    withClockAndReset(clock, reset) {
      ...
      val rxPauseCounters = Wire(UInt(1.W))
      ...
      when (ucieStack) { 
        ...
        test.io.phy.rx.bits.valid := 0.U
        test.io.phy.rx.bits.data := 0.U.asTypeOf(test.io.phy.rx.bits.data)
      }.otherwise {
...
trait CanHavePeripheryUciephyTest { this: BaseSubsystem =>
  ...
  val uciephy = p(UciephyTestKey) match {
    case Some(params) => {
      ...
      pbus.coupleTo(portName) { uciephy.node := TLBuffer() := TLFragmenter(pbus.beatBytes, pbus.blockBytes) := TLBuffer() := _ }
